"""Trains ml_classifier.reference's MLP with grouped k-fold
cross-validation (dataset.split.assign_folds, tools/build_folds.py)
instead of a fixed train/val/test split -- this is the primary
evaluation method: a fixed split permanently reserves most recordings
for validation/test and never trains on them, while k-fold rotates
every recording through the test role exactly once.

For each fold i: train on every other fold's rows, test on fold i's rows.
Fixed epoch count, no restore-best/early-stopping-on-val -- a pure
k-fold design has no separate val split at all, so that risk doesn't
even apply.

Usage (from 03.Reference):
    python -m tools.train_ml_classifier_kfold [--hidden 16] [--epochs 2000] [--lr 0.1]
"""
...
REPORT_OUTPUT = DATASET_DIR / "ml_classifier_kfold_report.json"
FINAL_WEIGHTS_OUTPUT = DATASET_DIR / "ml_classifier_weights.npz"
FINAL_REPORT_OUTPUT = DATASET_DIR / "ml_classifier_training_report.json"
...
def parse_args():
    ...
    parser.add_argument("--save-final", action="store_true",
                         help="after the k-fold comparison, train one more model on ALL rows "
                              "(no held-out data) and save it as the official "
                              "ml_classifier_weights.npz/training_report.json -- the k-fold "
                              "mean+/-std above is the honest generalization estimate for this "
                              "final model (standard practice: validate via CV, then ship a "
                              "model trained on every available source since more data only "
                              "helps once the approach is already validated)")
...
def main():
    ...
    if args.save_final:
        print(f"\n=== treino final sobre TODAS as {len(df)} linhas (sem retencao) ===")
        ...
        sample_weights_all = class_weights_from_counts(y_all, n_classes)[y_all]
